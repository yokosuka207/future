...
public class ResultSelect : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.mousePosition.y <= Screen.height / 5 + buttonMove && Input.mousePosition.y > 0 + buttonMove)
        {
        ...
        if (Input.mousePosition.y <= Screen.height / 5 + buttonMove && Input.mousePosition.y > 0 + buttonMove)
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                {
                    case 0:
                        sceneChenge.GetComponent<SceneChange>().PlayChengeScene(clearCheck.GetComponent<ClearCheck>().name);
                        break;
                    case 1:
